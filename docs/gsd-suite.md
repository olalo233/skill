...
本仓库公开后可通过 HTTPS 匿名读取，无需 token。SSH 仅适合已有对应认证的环境：
...
`npx skills add` 安装 skill 目录，不会自动安装`docs/gsd-project-policy.md`。要在整个项目采用套件约定，应将GSD 项目策略中的适用规则合并进现有 `AGENTS.md` / `CLAUDE.md`，保留项目原有约束；不要直接覆盖整份文件。`grill-me` 自身也包含同会话接续的调用边界。
...
- Git URL 路径可使用 `https://github.com/olalo233/skill`，可通过 HTTPS 匿名下载；网络受限时从已下载的本地 clone 导入。
